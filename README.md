...
В левой части страницы находится панель с тремя элементами:
...
2. **Ограничение по длине ответа** — числовое поле. Отправляется как `max_tokens`.
3. **Условие завершения ответа** — текстовое поле. Отправляется как `stop`.
